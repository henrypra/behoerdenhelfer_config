 ...
 * (`FormResponseDto`). Level 1 = the field types and keys the app understands today.
 * Anything the app must be updated for (a new field type, a new behavioral key)
 * bumps [CURRENT] — and content *using* the new feature automatically gets a higher
 * `minContentSchema` in the manifest, so older app versions skip it and keep their
 * last understood copy.
 */
